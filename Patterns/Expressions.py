...
class CallExpr(p.Pattern):
    ...
    def check(self, expr, ctx):
        if expr.opname == "call" and self.fcn.check(expr.x, ctx):
            # i don't give a fuck about arguments, only call expr matters
...
class CastExpr(p.Pattern):
    ...
    def __init__(self, inner, cast_type=None, may_missing=False):
        super(CastExpr, self).__init__()
        self.cast_type = cast_type
        self.inner = inner
        self.may_missing = may_missing
    ...
    def check(self, expr, ctx):
        ...
        elif self.may_missing:
            return self.inner.check(expr, ctx)
        return False
